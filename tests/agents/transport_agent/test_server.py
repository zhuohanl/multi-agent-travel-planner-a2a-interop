"""Unit tests for Transport Agent server."""
...
class TestHealthCheck:
    ...
    async def test_health_check_returns_json_response(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.transport_agent.server import health_check
    ...
    async def test_health_check_returns_correct_structure(self, mock_environment):
        ...
        assert data["agent_name"] == "Transport Agent"
...
class TestModuleLevelConfig:
    ...
    def test_host_from_environment(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.transport_agent.server import host
        assert host == "localhost"

    def test_port_from_environment(self, mock_environment):
        """Test that port is read from TRANSPORT_AGENT_PORT environment variable."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.transport_agent.server import port
        assert port == 10010
...
class TestA2AServer:
    ...
    def a2a_server_class(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.transport_agent.server import A2AServer
    ...
    def test_build_agent_executor_returns_executor(
        ...
        """Test that build_agent_executor returns an AgentFrameworkTransportAgentExecutor."""
        with patch.dict(os.environ, mock_environment):
            with patch(
                "src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"
            ) as mock_agent_class:
                ...
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10010)
                executor = server.build_agent_executor()

                assert isinstance(executor, AgentFrameworkTransportAgentExecutor)
    ...
    def test_build_agent_card_returns_agent_card(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10010)
    ...
    def test_build_agent_card_skill_has_correct_id(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                ...
                assert skill.id == "search_transport"
    ...
    def test_build_agent_card_skill_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                ...
                assert skill.name == "Search Transport Options"
    ...
    def test_build_agent_card_skill_has_tags(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                ...
                assert "transport-search" in skill.tags
                assert "travel-planning" in skill.tags
                assert "flights" in skill.tags
    ...
    def test_server_reuses_shared_state(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                server1 = a2a_server_class(mock_httpx_client, host="localhost", port=10010)
                task_store_1 = a2a_server_class._task_store

                server2 = a2a_server_class(mock_httpx_client, host="localhost", port=10010)
...
class TestLifespan:
    ...
    async def test_lifespan_sets_app_state(self, mock_environment, a2a_server_class):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                from src.agents.transport_agent.server import lifespan
...
class TestStarletteApp:
    ...
    def test_app_has_health_route(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.transport_agent.server import app
    ...
    def test_health_endpoint_with_test_client(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                import src.agents.transport_agent.server as server_module
...
class TestAgentCardDescription:
    ...
    def test_agent_card_description_mentions_flights(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions flights."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10010)
                card = server.build_agent_card()

                assert "flights" in card.description.lower()

    def test_agent_card_description_mentions_trains(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions trains."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10010)
                card = server.build_agent_card()

                assert "trains" in card.description.lower()

    def test_skill_description_mentions_transfers(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions airport/station transfers."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10010)
                card = server.build_agent_card()
                ...
                assert "transfer" in skill.description.lower()

    def test_skill_examples_cover_different_transport_types(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill examples cover different transport types."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.transport_agent.agent_executor.AgentFrameworkTransportAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10010)
                card = server.build_agent_card()
                ...
                transport_types = ["flight", "train", "airport", "transit"]
                mentioned = sum(1 for t in transport_types if t in examples_text)
                assert mentioned >= 2
