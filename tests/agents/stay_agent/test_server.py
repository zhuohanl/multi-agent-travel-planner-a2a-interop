"""Unit tests for Stay Agent server."""
...
class TestHealthCheck:
    ...
    async def test_health_check_returns_json_response(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.stay_agent.server import health_check
    ...
    async def test_health_check_returns_correct_structure(self, mock_environment):
        ...
        assert data["agent_name"] == "Stay Agent"
...
class TestModuleLevelConfig:
    ...
    def test_host_from_environment(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.stay_agent.server import host
        assert host == "localhost"

    def test_port_from_environment(self, mock_environment):
        """Test that port is read from STAY_AGENT_PORT environment variable."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.stay_agent.server import port
        assert port == 10009
...
class TestA2AServer:
    ...
    def a2a_server_class(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.stay_agent.server import A2AServer
    ...
    def test_build_agent_executor_returns_executor(
        ...
        """Test that build_agent_executor returns an AgentFrameworkStayAgentExecutor."""
        with patch.dict(os.environ, mock_environment):
            with patch(
                "src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"
            ) as mock_agent_class:
                ...
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10009)
                executor = server.build_agent_executor()

                assert isinstance(executor, AgentFrameworkStayAgentExecutor)
    ...
    def test_build_agent_card_returns_agent_card(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10009)
    ...
    def test_build_agent_card_skill_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"):
                ...
                assert skill.name == "Search Accommodations"
    ...
    def test_build_agent_card_skill_has_tags(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"):
                ...
                assert "stay-search" in skill.tags
                assert "travel-planning" in skill.tags
                assert "accommodations" in skill.tags
    ...
    def test_server_reuses_shared_state(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"):
                server1 = a2a_server_class(mock_httpx_client, host="localhost", port=10009)
                task_store_1 = a2a_server_class._task_store

                server2 = a2a_server_class(mock_httpx_client, host="localhost", port=10009)
...
class TestLifespan:
    ...
    async def test_lifespan_sets_app_state(self, mock_environment, a2a_server_class):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"):
                from src.agents.stay_agent.server import lifespan
...
class TestStarletteApp:
    ...
    def test_app_has_health_route(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.stay_agent.server import app
    ...
    def test_health_endpoint_with_test_client(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"):
                import importlib
                import src.agents.stay_agent.server as server_module
...
class TestAgentCardDescription:
    ...
    def test_agent_card_description_mentions_accommodations(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions accommodations."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10009)
                card = server.build_agent_card()

                assert "accommodations" in card.description.lower()

    def test_agent_card_description_mentions_neighborhoods(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions neighborhoods."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10009)
                card = server.build_agent_card()

                assert "neighborhoods" in card.description.lower()

    def test_skill_description_mentions_accommodations_count(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions finding 4-6 accommodations."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10009)
                card = server.build_agent_card()
                ...
                assert "4-6" in skill.description

    def test_skill_examples_cover_different_destinations(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill examples cover multiple destinations."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.stay_agent.agent_executor.AgentFrameworkStayAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10009)
                card = server.build_agent_card()
                ...
                destinations = ["Tokyo", "Paris", "Rome", "Singapore"]
                mentioned = sum(1 for d in destinations if d in examples_text)
                assert mentioned >= 2
