"""Unit tests for Route Agent server."""
...
class TestHealthCheck:
    ...
    async def test_health_check_returns_json_response(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.route_agent.server import health_check
...
class TestModuleLevelConfig:
    ...
    def test_host_from_environment(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.route_agent.server import host
        assert host == "localhost"

    def test_port_from_environment(self, mock_environment):
        """Test that port is read from ROUTE_AGENT_PORT environment variable."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.route_agent.server import port
        assert port == 10012
...
class TestA2AServer:
    ...
    def a2a_server_class(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.route_agent.server import A2AServer
    ...
    def test_build_agent_executor_returns_executor(
        ...
        """Test that build_agent_executor returns an AgentFrameworkRouteAgentExecutor."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.route_agent.agent_executor import (
                AgentFrameworkRouteAgentExecutor,
            )

            server = a2a_server_class(mock_httpx_client, host="localhost", port=10012)
            executor = server.build_agent_executor()

            assert isinstance(executor, AgentFrameworkRouteAgentExecutor)
    ...
    def test_build_agent_card_skill_has_correct_id(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert skill.id == "create_itinerary"
    ...
    def test_build_agent_card_skill_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert skill.name == "Create Day-by-Day Itinerary"
    ...
    def test_build_agent_card_skill_has_tags(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert "itinerary" in skill.tags
            assert "planning" in skill.tags
            assert "route" in skill.tags
    ...
    def test_server_reuses_shared_state(
        ...
        with patch.dict(os.environ, mock_environment):
            server1 = a2a_server_class(mock_httpx_client, host="localhost", port=10012)
            task_store_1 = a2a_server_class._task_store

            server2 = a2a_server_class(mock_httpx_client, host="localhost", port=10012)
...
class TestLifespan:
    ...
    async def test_lifespan_sets_app_state(self, mock_environment, a2a_server_class):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.route_agent.server import lifespan
...
class TestStarletteApp:
    ...
    def test_app_has_health_route(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.route_agent.server import app
    ...
    def test_health_endpoint_with_test_client(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            import src.agents.route_agent.server as server_module
...
class TestAgentCardDescription:
    ...
    def test_agent_card_description_mentions_itinerary(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions itinerary."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10012)
            card = server.build_agent_card()
    ...
    def test_agent_card_description_mentions_discovery(
        ...
        """Test that agent card description mentions discovery results."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10012)
            card = server.build_agent_card()
    ...
    def test_skill_description_mentions_day_by_day(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions day-by-day planning."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10012)
            card = server.build_agent_card()
            skill = card.skills[0]

            assert "day" in skill.description.lower()

    def test_skill_description_mentions_time_slots(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions time slots."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10012)
            card = server.build_agent_card()
            skill = card.skills[0]

            assert "slot" in skill.description.lower() or "time" in skill.description.lower()

    def test_skill_examples_cover_itinerary_scenarios(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill examples cover itinerary scenarios."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10012)
            card = server.build_agent_card()
            skill = card.skills[0]
            ...
            # Check for itinerary-related examples
            itinerary_terms = ["itinerary", "day", "plan", "schedule", "daily"]
            mentioned = sum(1 for term in itinerary_terms if term in examples_text)
            assert mentioned >= 1
