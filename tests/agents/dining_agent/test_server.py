"""Unit tests for Dining Agent server."""
...
class TestHealthCheck:
    ...
    async def test_health_check_returns_json_response(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.dining_agent.server import health_check
    ...
    async def test_health_check_returns_correct_structure(self, mock_environment):
        ...
        assert data["agent_name"] == "Dining Agent"
...
class TestModuleLevelConfig:
    ...
    def test_host_from_environment(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.dining_agent.server import host
        assert host == "localhost"

    def test_port_from_environment(self, mock_environment):
        """Test that port is read from DINING_AGENT_PORT environment variable."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.dining_agent.server import port
        assert port == 10017
...
class TestA2AServer:
    ...
    def a2a_server_class(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.dining_agent.server import A2AServer
    ...
    def test_build_agent_executor_returns_executor(
        ...
        """Test that build_agent_executor returns an AgentFrameworkDiningAgentExecutor."""
        with patch.dict(os.environ, mock_environment):
            with patch(
                "src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"
            ) as mock_agent_class:
                ...
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10017)
                executor = server.build_agent_executor()

                assert isinstance(executor, AgentFrameworkDiningAgentExecutor)
    ...
    def test_build_agent_card_returns_agent_card(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10017)
    ...
    def test_build_agent_card_skill_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                ...
                assert skill.name == "Search Dining Options"
    ...
    def test_build_agent_card_skill_has_tags(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                ...
                assert "dining" in skill.tags
                assert "travel-planning" in skill.tags
                assert "dietary" in skill.tags
    ...
    def test_server_reuses_shared_state(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                server1 = a2a_server_class(mock_httpx_client, host="localhost", port=10017)
                task_store_1 = a2a_server_class._task_store

                server2 = a2a_server_class(mock_httpx_client, host="localhost", port=10017)
...
class TestLifespan:
    ...
    async def test_lifespan_sets_app_state(self, mock_environment, a2a_server_class):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                from src.agents.dining_agent.server import lifespan
...
class TestStarletteApp:
    ...
    def test_app_has_health_route(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.dining_agent.server import app
    ...
    def test_health_endpoint_with_test_client(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                import importlib
                import src.agents.dining_agent.server as server_module
...
class TestAgentCardDescription:
    ...
    def test_agent_card_description_mentions_restaurants(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions restaurants."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10017)
                card = server.build_agent_card()

                assert "restaurants" in card.description.lower()

    def test_agent_card_description_mentions_dietary(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions dietary constraints."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10017)
                card = server.build_agent_card()

                assert "dietary" in card.description.lower()

    def test_skill_description_mentions_restaurants(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions restaurants."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10017)
                card = server.build_agent_card()
                ...
                assert "restaurants" in skill.description.lower()

    def test_skill_description_mentions_dietary_constraints(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions dietary constraints."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10017)
                card = server.build_agent_card()
                ...
                assert "dietary" in skill.description.lower() or "vegetarian" in skill.description.lower()

    def test_skill_examples_cover_dietary_options(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill examples cover dietary options."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.dining_agent.agent_executor.AgentFrameworkDiningAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10017)
                card = server.build_agent_card()
                ...
                # Check for dietary-related examples
                dietary_terms = ["vegetarian", "halal", "vegan", "kosher", "gluten"]
                mentioned = sum(1 for term in dietary_terms if term in examples_text)
                assert mentioned >= 1
