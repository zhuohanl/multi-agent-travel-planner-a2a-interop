"""Unit tests for Booking Agent server."""
...
class TestHealthCheck:
    ...
    async def test_health_check_returns_json_response(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.booking_agent.server import health_check
    ...
    async def test_health_check_returns_correct_structure(self, mock_environment):
        ...
        assert data["agent_name"] == "Booking Agent"
...
class TestModuleLevelConfig:
    ...
    def test_host_from_environment(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.booking_agent.server import host
        assert host == "localhost"

    def test_port_from_environment(self, mock_environment):
        """Test that port is read from BOOKING_AGENT_PORT environment variable."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.booking_agent.server import port
        assert port == 10014
...
class TestA2AServer:
    ...
    def a2a_server_class(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.booking_agent.server import A2AServer
    ...
    def test_build_agent_executor_returns_executor(
        ...
        """Test that build_agent_executor returns an AgentFrameworkBookingAgentExecutor."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.booking_agent.agent_executor import (
                AgentFrameworkBookingAgentExecutor,
            )

            server = a2a_server_class(mock_httpx_client, host="localhost", port=10014)
            executor = server.build_agent_executor()

            assert isinstance(executor, AgentFrameworkBookingAgentExecutor)
    ...
    def test_build_agent_card_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert card.name == "Booking Agent"
    ...
    def test_build_agent_card_skill_has_correct_id(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert skill.id == "manage_bookings"
    ...
    def test_build_agent_card_skill_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert skill.name == "Manage Travel Bookings"
    ...
    def test_build_agent_card_skill_has_tags(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert "booking" in skill.tags
            assert "travel" in skill.tags
    ...
    def test_server_reuses_shared_state(
        ...
        with patch.dict(os.environ, mock_environment):
            server1 = a2a_server_class(mock_httpx_client, host="localhost", port=10014)
            task_store_1 = a2a_server_class._task_store

            server2 = a2a_server_class(mock_httpx_client, host="localhost", port=10014)
...
class TestLifespan:
    ...
    async def test_lifespan_sets_app_state(self, mock_environment, a2a_server_class):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.booking_agent.server import lifespan
...
class TestStarletteApp:
    ...
    def test_app_has_health_route(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.booking_agent.server import app
    ...
    def test_health_endpoint_with_test_client(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            import src.agents.booking_agent.server as server_module
...
class TestAgentCardDescription:
    ...
    def test_agent_card_description_mentions_booking(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions booking."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10014)
            card = server.build_agent_card()

            assert "booking" in card.description.lower()

    def test_agent_card_description_mentions_actions(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions the actions."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10014)
            card = server.build_agent_card()

            description_lower = card.description.lower()
            actions_mentioned = sum(1 for a in ["creation", "modification", "cancellation"]
                                    if a in description_lower)
            assert actions_mentioned >= 2

    def test_skill_description_mentions_actions(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions operating actions."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10014)
            card = server.build_agent_card()
            skill = card.skills[0]
            ...
            assert "create" in description_lower
            assert "modify" in description_lower
            assert "cancel" in description_lower

    def test_skill_examples_cover_different_actions(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill examples cover different actions."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10014)
            card = server.build_agent_card()
            skill = card.skills[0]
            ...
            # Check for action-related examples
            action_terms = ["create", "modify", "cancel"]
            mentioned = sum(1 for term in action_terms if term in examples_text)
            assert mentioned >= 2
