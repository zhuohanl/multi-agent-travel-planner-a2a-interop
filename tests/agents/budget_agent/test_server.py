"""Unit tests for Budget Agent server."""
...
class TestHealthCheck:
    ...
    async def test_health_check_returns_json_response(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.budget_agent.server import health_check
    ...
    async def test_health_check_returns_correct_structure(self, mock_environment):
        ...
        assert data["agent_name"] == "Budget Agent"
...
class TestModuleLevelConfig:
    ...
    def test_host_from_environment(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.budget_agent.server import host
        assert host == "localhost"

    def test_port_from_environment(self, mock_environment):
        """Test that port is read from BUDGET_AGENT_PORT environment variable."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.budget_agent.server import port
        assert port == 10013
...
class TestA2AServer:
    ...
    def a2a_server_class(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.budget_agent.server import A2AServer
    ...
    def test_build_agent_executor_returns_executor(
        ...
        """Test that build_agent_executor returns an AgentFrameworkBudgetAgentExecutor."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.budget_agent.agent_executor import (
                AgentFrameworkBudgetAgentExecutor,
            )

            server = a2a_server_class(mock_httpx_client, host="localhost", port=10013)
            executor = server.build_agent_executor()

            assert isinstance(executor, AgentFrameworkBudgetAgentExecutor)
    ...
    def test_build_agent_card_skill_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert skill.name == "Manage Travel Budget"
    ...
    def test_server_reuses_shared_state(
        ...
        with patch.dict(os.environ, mock_environment):
            server1 = a2a_server_class(mock_httpx_client, host="localhost", port=10013)
            task_store_1 = a2a_server_class._task_store

            server2 = a2a_server_class(mock_httpx_client, host="localhost", port=10013)
...
class TestLifespan:
    ...
    async def test_lifespan_sets_app_state(self, mock_environment, a2a_server_class):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.budget_agent.server import lifespan
...
class TestStarletteApp:
    ...
    def test_app_has_health_route(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.budget_agent.server import app
    ...
    def test_health_endpoint_with_test_client(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            import src.agents.budget_agent.server as server_module
...
class TestAgentCardDescription:
    ...
    def test_agent_card_description_mentions_modes(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions the modes."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10013)
            card = server.build_agent_card()

            description_lower = card.description.lower()
            modes_mentioned = sum(1 for m in ["propose", "validate", "track", "reallocate"]
                                 if m in description_lower)
            assert modes_mentioned >= 2

    def test_skill_description_mentions_modes(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions operating modes."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10013)
            card = server.build_agent_card()
            skill = card.skills[0]
            ...
            assert "propose" in description_lower
            assert "validate" in description_lower
            assert "track" in description_lower
            assert "reallocate" in description_lower

    def test_skill_description_mentions_idempotent(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions idempotency."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10013)
            card = server.build_agent_card()
            skill = card.skills[0]

            assert "idempotent" in skill.description.lower()

    def test_skill_examples_cover_different_modes(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill examples cover different modes."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10013)
            card = server.build_agent_card()
            skill = card.skills[0]
            ...
            # Check for mode-related examples
            mode_terms = ["propose", "validate", "track", "realloc"]
            mentioned = sum(1 for term in mode_terms if term in examples_text)
            assert mentioned >= 2
