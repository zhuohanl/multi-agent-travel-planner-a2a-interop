"""Unit tests for POI Search Agent server."""
...
class TestHealthCheck:
    ...
    async def test_health_check_returns_json_response(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.poi_search_agent.server import health_check
...
class TestModuleLevelConfig:
    ...
    def test_host_from_environment(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.poi_search_agent.server import host
        assert host == "localhost"

    def test_port_from_environment(self, mock_environment):
        """Test that port is read from POI_SEARCH_AGENT_PORT environment variable."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.poi_search_agent.server import port
        assert port == 10008
...
class TestA2AServer:
    ...
    def a2a_server_class(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.poi_search_agent.server import A2AServer
            # Reset class state
    ...
    def test_build_agent_executor_returns_executor(
        ...
        """Test that build_agent_executor returns an AgentFrameworkPOISearchAgentExecutor."""
        with patch.dict(os.environ, mock_environment):
            # Mock the agent that requires Azure credentials
            with patch(
                "src.agents.poi_search_agent.agent_executor.AgentFrameworkPOISearchAgent"
            ) as mock_agent_class:
                mock_agent_class.return_value = MagicMock()

                from src.agents.poi_search_agent.agent_executor import (
                    AgentFrameworkPOISearchAgentExecutor,
                )

                server = a2a_server_class(mock_httpx_client, host="localhost", port=10008)
                executor = server.build_agent_executor()

                assert isinstance(executor, AgentFrameworkPOISearchAgentExecutor)
    ...
    def test_build_agent_card_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch(
                ...
                assert card.name == "POI Search Agent"
    ...
    def test_build_agent_card_has_correct_url(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch(
                ...
                # Reset state again for clean URL test
    ...
    def test_build_agent_card_skill_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch(
                ...
                assert skill.name == "Search Points of Interest"
    ...
    def test_build_agent_card_skill_has_tags(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch(
                ...
                assert "poi-search" in skill.tags
    ...
    def test_server_reuses_shared_state(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch(
                ...
                server1 = a2a_server_class(mock_httpx_client, host="localhost", port=10008)
                task_store_1 = a2a_server_class._task_store

                server2 = a2a_server_class(mock_httpx_client, host="localhost", port=10008)
    ...
    async def test_lifespan_sets_app_state(self, mock_environment, a2a_server_class):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch(
                ...
                from src.agents.poi_search_agent.server import lifespan
    ...
    async def test_lifespan_closes_httpx_client_on_exit(self, mock_environment, a2a_server_class):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch(
                ...
                # Context exited without error - cleanup happened
                assert True
...
class TestStarletteApp:
    ...
    def test_app_has_health_route(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.poi_search_agent.server import app
    ...
    def test_health_endpoint_with_test_client(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch(
                ...
                import importlib
                import src.agents.poi_search_agent.server as server_module
...
class TestAgentCardDescription:
    ...
    def test_agent_card_description_mentions_poi(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions points of interest."""
        with patch.dict(os.environ, mock_environment):
            with patch(
                "src.agents.poi_search_agent.agent_executor.AgentFrameworkPOISearchAgent"
            ):
                ...
                assert "points of interest" in card.description.lower()

    def test_skill_description_mentions_poi_count(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions finding 6-10 POIs."""
        with patch.dict(os.environ, mock_environment):
            with patch(
                "src.agents.poi_search_agent.agent_executor.AgentFrameworkPOISearchAgent"
            ):
                ...
                assert "6-10" in skill.description
