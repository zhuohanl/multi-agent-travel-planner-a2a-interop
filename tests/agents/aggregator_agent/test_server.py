"""Unit tests for Aggregator Agent server."""
...
class TestHealthCheck:
    ...
    async def test_health_check_returns_json_response(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.aggregator_agent.server import health_check
    ...
    async def test_health_check_returns_correct_structure(self, mock_environment):
        ...
        assert data["agent_name"] == "Aggregator Agent"
...
class TestModuleLevelConfig:
    ...
    def test_host_from_environment(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.aggregator_agent.server import host
        assert host == "localhost"

    def test_port_from_environment(self, mock_environment):
        """Test that port is read from AGGREGATOR_AGENT_PORT environment variable."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.aggregator_agent.server import port
        assert port == 10015
...
class TestA2AServer:
    ...
    def a2a_server_class(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.aggregator_agent.server import A2AServer
    ...
    def test_build_agent_executor_returns_executor(
        ...
        """Test that build_agent_executor returns an AgentFrameworkAggregatorAgentExecutor."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.aggregator_agent.agent_executor import (
                AgentFrameworkAggregatorAgentExecutor,
            )

            server = a2a_server_class(mock_httpx_client, host="localhost", port=10015)
            executor = server.build_agent_executor()

            assert isinstance(executor, AgentFrameworkAggregatorAgentExecutor)
    ...
    def test_build_agent_card_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert card.name == "Aggregator Agent"
    ...
    def test_build_agent_card_skill_has_correct_id(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert skill.id == "aggregate_discovery"
    ...
    def test_build_agent_card_skill_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert skill.name == "Aggregate Discovery Results"
    ...
    def test_build_agent_card_skill_has_tags(
        ...
        with patch.dict(os.environ, mock_environment):
            ...
            assert "aggregation" in skill.tags
            assert "discovery" in skill.tags
    ...
    def test_server_reuses_shared_state(
        ...
        with patch.dict(os.environ, mock_environment):
            server1 = a2a_server_class(mock_httpx_client, host="localhost", port=10015)
            task_store_1 = a2a_server_class._task_store

            server2 = a2a_server_class(mock_httpx_client, host="localhost", port=10015)
...
class TestLifespan:
    ...
    async def test_lifespan_sets_app_state(self, mock_environment, a2a_server_class):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.aggregator_agent.server import lifespan
...
class TestStarletteApp:
    ...
    def test_app_has_health_route(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.aggregator_agent.server import app
    ...
    def test_health_endpoint_with_test_client(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            import src.agents.aggregator_agent.server as server_module
...
class TestAgentCardDescription:
    ...
    def test_agent_card_description_mentions_discovery(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions discovery."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10015)
            card = server.build_agent_card()

            assert "discovery" in card.description.lower()

    def test_agent_card_description_mentions_combining(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions combining results."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10015)
            card = server.build_agent_card()

            assert "combin" in card.description.lower()

    def test_agent_card_description_mentions_not_validating(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description clarifies it does not validate."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10015)
            card = server.build_agent_card()

            # Should mention that validation is done elsewhere
            assert "not validate" in card.description.lower() or "validator" in card.description.lower()

    def test_skill_description_mentions_multiple_agents(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions combining from multiple agents."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10015)
            card = server.build_agent_card()
            skill = card.skills[0]

            # Should mention various agent types
            description_lower = skill.description.lower()
            agent_types = ["poi", "stay", "transport", "events", "dining"]
            mentioned = sum(1 for t in agent_types if t in description_lower)
            assert mentioned >= 3  # At least mention 3 agent types

    def test_skill_examples_cover_aggregation_scenarios(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill examples cover aggregation scenarios."""
        with patch.dict(os.environ, mock_environment):
            server = a2a_server_class(mock_httpx_client, host="localhost", port=10015)
            card = server.build_agent_card()
            skill = card.skills[0]
            ...
            # Check for aggregation-related examples
            aggregation_terms = ["combine", "aggregate", "merge", "discovery"]
            mentioned = sum(1 for term in aggregation_terms if term in examples_text)
            assert mentioned >= 1
