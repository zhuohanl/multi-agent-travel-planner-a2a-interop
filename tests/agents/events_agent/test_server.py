"""Unit tests for Events Agent server."""
...
class TestHealthCheck:
    ...
    async def test_health_check_returns_json_response(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.events_agent.server import health_check
    ...
    async def test_health_check_returns_correct_structure(self, mock_environment):
        ...
        assert data["agent_name"] == "Events Agent"
...
class TestModuleLevelConfig:
    ...
    def test_host_from_environment(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.events_agent.server import host
        assert host == "localhost"

    def test_port_from_environment(self, mock_environment):
        """Test that port is read from EVENTS_AGENT_PORT environment variable."""
        with patch.dict(os.environ, mock_environment):
            from src.agents.events_agent.server import port
        assert port == 10011
...
class TestA2AServer:
    ...
    def a2a_server_class(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.events_agent.server import A2AServer
    ...
    def test_build_agent_executor_returns_executor(
        ...
        """Test that build_agent_executor returns an AgentFrameworkEventsAgentExecutor."""
        with patch.dict(os.environ, mock_environment):
            with patch(
                "src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"
            ) as mock_agent_class:
                ...
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10011)
                executor = server.build_agent_executor()

                assert isinstance(executor, AgentFrameworkEventsAgentExecutor)
    ...
    def test_build_agent_card_returns_agent_card(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10011)
    ...
    def test_build_agent_card_skill_has_correct_name(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"):
                ...
                assert skill.name == "Search Events"
    ...
    def test_build_agent_card_skill_has_tags(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"):
                ...
                assert "events" in skill.tags
                assert "travel-planning" in skill.tags
                assert "festivals" in skill.tags
    ...
    def test_server_reuses_shared_state(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"):
                server1 = a2a_server_class(mock_httpx_client, host="localhost", port=10011)
                task_store_1 = a2a_server_class._task_store

                server2 = a2a_server_class(mock_httpx_client, host="localhost", port=10011)
...
class TestLifespan:
    ...
    async def test_lifespan_sets_app_state(self, mock_environment, a2a_server_class):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"):
                from src.agents.events_agent.server import lifespan
...
class TestStarletteApp:
    ...
    def test_app_has_health_route(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            from src.agents.events_agent.server import app
    ...
    def test_health_endpoint_with_test_client(self, mock_environment):
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"):
                import importlib
                import src.agents.events_agent.server as server_module
...
class TestAgentCardDescription:
    ...
    def test_agent_card_description_mentions_events(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions events."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10011)
                card = server.build_agent_card()

                assert "events" in card.description.lower()

    def test_agent_card_description_mentions_festivals(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that agent card description mentions festivals."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10011)
                card = server.build_agent_card()

                assert "festivals" in card.description.lower()

    def test_skill_description_mentions_events(
        self, mock_httpx_client, a2a_server_class, mock_environment
    ):
        """Test that skill description mentions events."""
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"):
                server = a2a_server_class(mock_httpx_client, host="localhost", port=10011)
                card = server.build_agent_card()
                ...
                assert "events" in skill.description.lower()
    ...
    def test_skill_examples_cover_different_destinations(
        ...
        with patch.dict(os.environ, mock_environment):
            with patch("src.agents.events_agent.agent_executor.AgentFrameworkEventsAgent"):
                ...
                destinations = ["Tokyo", "Kyoto", "Paris", "London"]
