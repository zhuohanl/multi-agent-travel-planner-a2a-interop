...
class AgentFrameworkStayAgent(BaseAgentFrameworkAgent):
    """Wraps Microsoft Agent Framework to search for accommodations at travel destinations.

    Implements Detect & Adapt behavior:
    - Specific input (hotel name): validates and enriches details
    - Vague input: returns 3-5 neighborhood/stay options
    - Mixed input with constraints: respects user preferences

    Supports two modes:
    - Q&A mode (mode="qa"): Answers questions about accommodations, returns text response
    ...
    def parse_response(self, message: Any) -> dict[str, Any]:
        ...
        - Planning mode: structured output in 'stay_output' field
        """
        try:
            stay_response = StayResponse.model_validate_json(message)

            # Q&A mode: return text response (single-turn, always complete)
            # Per design doc: Q&A mode sets is_task_complete=True
            if self._is_qa_mode and stay_response.response:
                logger.debug("Returning Q&A mode response")
                return {
                    'is_task_complete': True,
                    'require_user_input': False,
                    'content': stay_response.response,
                }

            # Planning mode: StayResponse.response is used when required inputs are missing.
            # This lets the agent ask follow-up questions while keeping output structured.
            if stay_response.response:
                return {
            ...
            # StayResponse.stay_output is the successful result payload.
            if stay_response.stay_output:
                return {
                    'is_task_complete': True,
                    'require_user_input': False,
                    'content': stay_response.stay_output.model_dump_json(),
                }

            return {
                'is_task_complete': False,
                'require_user_input': True,
                'content': 'Please provide the destination city to search for stays.',
            }

        except Exception as e:
            logger.error(f"Failed to parse StayResponse: {e}")
