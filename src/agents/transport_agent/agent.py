...
from src.shared.models import TransportResponse

logger = logging.getLogger(__name__)
load_dotenv()


class AgentFrameworkTransportAgent(BaseAgentFrameworkAgent):
    """Wraps Microsoft Agent Framework to search for transport options.

    Implements Detect & Adapt behavior:
    - Specific input (flight number, train): validates and enriches details
    - Vague input: returns 3-5 transport options across modes
    - Mixed input with constraints: respects user preferences (scenic, budget)

    Supports two modes:
    - Q&A mode (mode="qa"): Answers questions about transport, returns text response
    ...
    def parse_response(self, message: Any) -> dict[str, Any]:
        ...
        - Planning mode: structured output in 'transport_output' field
        """
        try:
            transport_response = TransportResponse.model_validate_json(message)

            # Q&A mode: return text response (single-turn, always complete)
            # Per design doc: Q&A mode sets is_task_complete=True
            if self._is_qa_mode and transport_response.response:
                logger.debug("Returning Q&A mode response")
                return {
                    'is_task_complete': True,
                    'require_user_input': False,
                    'content': transport_response.response,
                }

            # Planning mode: TransportResponse.response is used when required inputs are missing.
            # This lets the agent ask follow-up questions while keeping output structured.
            if transport_response.response:
                return {
            ...
            # TransportResponse.transport_output is the successful result payload.
            if transport_response.transport_output:
                return {
                    'is_task_complete': True,
                    'require_user_input': False,
                    'content': transport_response.transport_output.model_dump_json(),
                }

            return {
                'is_task_complete': False,
                'require_user_input': True,
                'content': 'Please provide origin and destination cities for transport search.',
            }

        except Exception as e:
            logger.error(f"Failed to parse TransportResponse: {e}")
