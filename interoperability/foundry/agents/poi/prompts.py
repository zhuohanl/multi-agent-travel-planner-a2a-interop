"""
Prompts module for POI Search Agent.

This module provides access to the POI Search agent's system prompt
by using the extract_agent module to load from the canonical source.

The prompt content comes from src/prompts/search.txt and is
...
def get_system_prompt() -> str:
    """Get the POI Search agent's system prompt.

    Extracts the prompt from src/prompts/search.txt via the
    ...
    """
    return extract_system_prompt("src/agents/poi_search_agent")
